...
class Getthedata {
private:
    std::string api_base = "https://api.getthedata.com";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("api.getthedata.com"));
...
public:
    ...
    pplx::task<json::value> OSGB36_to_WGS84(int easting,int northing){
        return make_api_call("/bng2latlong/" + std::to_string(easting) + "/" + std::to_string(northing),"GET");
    }
    
    pplx::task<json::value> get_postcode(const std::string& postcode){
        return make_api_call("/postcode/" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(postcode))) + "&format=json","GET");
    }
